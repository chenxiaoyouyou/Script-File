        ...
        if char == '”':
            if len(yinhao_pairs) > 0:
                ...
                if last_item['c'] in {"“", "‘", "\u301D"}:
                    ...
                    error_list.append(
                        [page, number, last_item["i"], last_item["c"], "成对标点符号格式必须一致", 1])
                    yinhao_pairs.pop()
                else:
                    error_list.append([page, number, i, char, "需补充“", 1])
            else:
...
if __name__ == '__main__':
    # check_punc(text)‘’
    # '^[a-zA-Z ]*[0-9\.∶-≦≈÷=∑∏≮∴＝﹣﹢﹤≤·＜＋/≡＞*㏒－∵+≠﹥≧≒≯㏑×≥∅\(\)\[\]\{\}｛｝（）］［]+$
    text1 = """然而他的书没有获得真正的成功，读者的口味已经不习惯如此干巴巴的、没有用油和伤感烹调出来的菜肴，他必须为他自己塑造的形象想出一个读者群，完全往前，在另一个世纪，一种精英，“happy few”（幸运的少数），1890年或1900年那一代人。但同时代人'的冷漠对司汤达并没有造成多么严重的伤害。“别人与我何干？”司汤达写作只为了自己。这个日渐衰老的伊壁鸠鲁主义者发现了一种新的、最后的也是最精致的乐趣：在上面阁楼里点上他木桌上的两根蜡烛，去写或口授，这种与他的灵魂和他的思想私下里的、完全内心的自我交谈在他迟暮之年，比所有的女人和朋友，比弗依咖啡馆，比沙龙中的讨论，甚至比音乐都更为重要。在孤独中的享受和在享受中的孤独——这个五十岁的人终于在艺术中发现了他的＂最初的和最古老的原始理想。"""
    text2 = '<都是>'
