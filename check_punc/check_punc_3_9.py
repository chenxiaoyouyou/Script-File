...
def check(text, number, page):
    ...
    for i, char in enumerate(text):
        ...
        if char == '）':
            ...
            #         flag_start = False
            #         continue
            is_order = is_order_number(i, text, kuohao_pairs)
...
def c_yinhao_nesting_check(yinhao_list, label="“"):
    """中文引号嵌套检查"""
    if label == "“":
        left1 = "“"
        right1 = "”"
...
def is_order_number(index, text, kuohao_pairs):
    ...
    if len(kuohao_pairs) > 0:
        if text[index] == ")":
            ...
            target = ""
        if kuohao_pairs[-1]['c'] == target:
            slice_str = text[kuohao_pairs[-1]["i"]:index+1]
            res = pattern.match(slice_str)
            if not res:
                return False
    ...
    if pattern_kuohao_2.match(newtext):
        ...
        if len(pattern_kuohao_2.match(newtext).group().strip().strip(")").strip()) <= 2:
            ...
            #     else:
            #         target = ""
            #     if pattern_kuohao_2.match(newtext).group()[-1] == target:
            #         kuohao_pairs.pop()
            return True
        ...
        if len(pattern_kuohao_1.match(newtext).group().strip().strip(")").strip()) <= 2:
            ...
            #     else:
            #         target = ""
            #     if pattern_kuohao_1.match(newtext).group()[-1] == target:
            #         kuohao_pairs.pop()
            return True
...
def is_compare(i, text):
    ...
    # print(left)
    right = text[i+1:]
    pa_digit = re.compile(pa_digit)
    pa_eng = re.compile(pa_eng)
    eng_left = pa_eng.match(left)
    eng_right = pa_eng.match(right)
    # print(132)
    if eng_left:
        eng_left = eng_left.group().strip()
        if len(eng_left) == 1:
            eng_left = True
        else:
            eng_left = False
    if eng_right:
        eng_right = eng_right.group().strip()
        if len(eng_right) == 1:
            eng_right = True
        else:
            eng_right = False
    digit_left = pa_digit.match(left)
    digit_right = pa_digit.match(right)

    if eng_left and digit_right:
        return True
    if eng_left and eng_right:
        return True
    if digit_left and digit_right:
        return True
    if digit_left and eng_right:
        return True
